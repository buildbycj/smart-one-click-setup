...
class ImportHelper {
	...
	private static function register_cache_key( $cache_key ) {
		$tracked_keys = get_option( 'smartocs_api_demo_cache_keys', array() );
		if ( ! is_array( $tracked_keys ) ) {
			$tracked_keys = array();
		}
		if ( ! in_array( $cache_key, $tracked_keys, true ) ) {
			$tracked_keys[] = $cache_key;
			update_option( 'smartocs_api_demo_cache_keys', $tracked_keys, false );
		}
	}
	...
	public static function clear_api_cache( $api_url = '' ) {
		if ( ! empty( $api_url ) ) {
			$cache_key = 'smartocs_api_demos_' . md5( $api_url );
			...
			$tracked_keys = get_option( 'smartocs_api_demo_cache_keys', array() );
			if ( is_array( $tracked_keys ) ) {
				$tracked_keys = array_diff( $tracked_keys, array( $cache_key ) );
				update_option( 'smartocs_api_demo_cache_keys', $tracked_keys, false );
			}
		} else {
			// Clear all API caches using tracked keys.
			$tracked_keys = get_option( 'smartocs_api_demo_cache_keys', array() );
			if ( is_array( $tracked_keys ) && ! empty( $tracked_keys ) ) {
				foreach ( $tracked_keys as $cache_key ) {
					// Delete transient (handles both database and object cache).
					delete_transient( $cache_key );
					// Also explicitly clear from object cache as a safety measure.
					wp_cache_delete( $cache_key, 'transient' );
				}
			}
			// Clear the tracked keys list.
			delete_option( 'smartocs_api_demo_cache_keys' );
			wp_cache_delete( 'smartocs_api_demo_cache_keys', 'options' );
		}
	}
	 ...
	 *
	 * This method:
	 * 1. Gets the base URL from filter 'smartocs/demo_api_base_url'
	...
	public static function fetch_from_api( $base_url = '', $theme_name = '', $api_endpoint = 'demos.json' ) {
		...
		if ( empty( $base_url ) ) {
			$base_url = Helpers::apply_filters( 'smartocs/demo_api_base_url', '' );
		}
		...
		if ( empty( $base_url ) ) {
			return new \WP_Error(
				'no_base_url',
				__( 'No API base URL provided. Use the smartocs/demo_api_base_url filter or pass it as a parameter.', 'smart-one-click-setup' )
			);
		}
		...
		Helpers::debug_log( 'API Debug - Base URL: ' . $base_url );
		Helpers::debug_log( 'API Debug - Theme Name: ' . $theme_name );
		Helpers::debug_log( 'API Debug - API URL: ' . $api_url );
		...
		if ( false !== $cached_demos ) {
			// Debug logging if WP_DEBUG is enabled.
			Helpers::debug_log( 'API Debug - Using cached demos. Count: ' . ( is_array( $cached_demos ) ? count( $cached_demos ) : 'N/A' ) );
			if ( is_array( $cached_demos ) && ! empty( $cached_demos ) ) {
				Helpers::debug_log( 'API Debug - Cached data preview: ' . wp_json_encode( array_slice( $cached_demos, 0, 1 ) ) );
			}
			...
			if ( ! $is_valid_cache ) {
				// Debug logging if WP_DEBUG is enabled.
				Helpers::debug_log( 'API Debug - Cached data is invalid or has no valid demos, clearing cache and fetching fresh' );
				delete_transient( $cache_key );
				wp_cache_delete( $cache_key, 'transient' );
				// Remove from tracked keys.
				$tracked_keys = get_option( 'smartocs_api_demo_cache_keys', array() );
		...
		// Fetch from API.
		$timeout = Helpers::apply_filters( 'smartocs/demo_api_timeout', 15 );
		$response = wp_remote_get(
			$api_url,
			array(
				'timeout' => $timeout,
				'sslverify' => Helpers::apply_filters( 'smartocs/demo_api_sslverify', true ),
			)
		);
		...
		if ( is_wp_error( $response ) ) {
			...
			// Debug logging if WP_DEBUG is enabled.
			Helpers::debug_log( 'API Error - Fetch failed: ' . $error_message );
			return new \WP_Error(
				'api_fetch_error',
				sprintf(
		...
		// Debug logging if WP_DEBUG is enabled.
		Helpers::debug_log( 'API Debug - Response Code: ' . $response_code );
		if ( 200 !== $response_code ) {
			// Debug logging if WP_DEBUG is enabled.
			Helpers::debug_log( 'API Error - Non-200 response code: ' . $response_code );
			return new \WP_Error(
				'api_response_error',
				sprintf(
		...
		// Debug logging if WP_DEBUG is enabled.
		Helpers::debug_log( 'API Debug - Response body length: ' . strlen( $body ) );
		Helpers::debug_log( 'API Debug - Response body preview: ' . substr( $body, 0, 200 ) );
		if ( empty( $body ) ) {
			// Debug logging if WP_DEBUG is enabled.
			Helpers::debug_log( 'API Error - Empty response body' );
			return new \WP_Error(
				'empty_response',
				sprintf(
		...
		if ( json_last_error() !== JSON_ERROR_NONE ) {
			...
			// Debug logging if WP_DEBUG is enabled.
			Helpers::debug_log( 'API Error - JSON parse failed: ' . $json_error );
			return new \WP_Error(
				'json_parse_error',
				sprintf(
		...
		// Debug logging if WP_DEBUG is enabled.
		Helpers::debug_log( 'API Debug - Parsed JSON type: ' . gettype( $demos_data ) );
		if ( is_array( $demos_data ) ) {
			Helpers::debug_log( 'API Debug - Parsed JSON keys: ' . implode( ', ', array_keys( $demos_data ) ) );
			Helpers::debug_log( 'API Debug - Parsed JSON count: ' . count( $demos_data ) );
		}

		// Cache the response (1 hour default).
		$cache_duration = Helpers::apply_filters( 'smartocs/demo_api_cache_duration', HOUR_IN_SECONDS );
		...
		// Debug logging if WP_DEBUG is enabled.
		Helpers::debug_log( 'API Debug - Demos registered: ' . ( $demos_after - $demos_before ) . ' (Total: ' . $demos_after . ')' );
	...
	private static function register_demos_from_api_response( $demos_data ) {
		// Debug logging if WP_DEBUG is enabled.
		Helpers::debug_log( 'API Debug - register_demos_from_api_response called with type: ' . gettype( $demos_data ) );
		if ( ! is_array( $demos_data ) ) {
			// Debug logging if WP_DEBUG is enabled.
			Helpers::debug_log( 'API Error - register_demos_from_api_response: data is not an array' );
			return;
		}
		...
		// Debug logging if WP_DEBUG is enabled.
		Helpers::debug_log( 'API Debug - Found ' . count( $demos ) . ' demo(s) in response' );
		...
		foreach ( $demos as $index => $demo ) {
			if ( ! is_array( $demo ) ) {
				// Debug logging if WP_DEBUG is enabled.
				Helpers::debug_log( 'API Debug - Skipping demo #' . $index . ': not an array' );
				$skipped_count++;
				continue;
			}
			...
			if ( empty( $name ) || ( empty( $zip_url ) && empty( $zip_path ) ) ) {
				// Debug logging if WP_DEBUG is enabled.
				Helpers::debug_log( 'API Debug - Skipping demo #' . $index . ': missing name or zip_url/zip_path. Name: ' . ( $name ?: 'empty' ) . ', zip_url: ' . ( $zip_url ?: 'empty' ) . ', zip_path: ' . ( $zip_path ?: 'empty' ) );
				$skipped_count++;
				continue;
			}
			...
			// Debug logging if WP_DEBUG is enabled.
			Helpers::debug_log( 'API Debug - Registered demo #' . $index . ': ' . $name );
		}

		// Debug logging if WP_DEBUG is enabled.
		Helpers::debug_log( 'API Debug - Registration complete. Registered: ' . $registered_count . ', Skipped: ' . $skipped_count );
	}
}
