...
class PluginSettingsImporter {
	...
	/**
	 * Check if a plugin is active without requiring core files.
	 *
	 * @param string $plugin_file Plugin file path.
	 * @return bool True if plugin is active, false otherwise.
	 */
	private static function is_plugin_active_custom( $plugin_file ) {
	...
	public static function import( $plugin_settings_import_file_path ) {
		...
		$log_section_title = esc_html__( 'Importing plugin settings', 'smart-one-click-setup' );
	...
	private static function import_plugin_settings( $data_file ) {
		...
		// Hook before import.
		Helpers::do_action( 'smartocs/plugin_settings_importer_before_import' );
		$data = Helpers::apply_filters( 'smartocs/before_plugin_settings_import_data', $data );
		...
		// Hook after import.
		Helpers::do_action( 'smartocs/plugin_settings_importer_after_import' );
	...
	private static function is_plugin_active( $plugin_slug ) {
		...
		if ( strpos( $plugin_slug, '/' ) !== false ) {
			return self::is_plugin_active_custom( $plugin_slug );
		}

		// Check active plugins by slug.
		$smartocs_active_plugins = get_option( 'active_plugins', array() );
		foreach ( $smartocs_active_plugins as $plugin_file ) {
		...
		$is_active = Helpers::apply_filters( 'smartocs/is_plugin_active_' . $plugin_slug, false, $plugin_slug );
		if ( $is_active ) {
			return true;
		}
	...
	private static function import_plugin_settings_data( $plugin_slug, $settings ) {
		...
		$imported = Helpers::apply_filters( 'smartocs/import_plugin_' . $plugin_slug . '_settings', false, $settings );
		...
		if ( $imported === true ) {
			...
			Helpers::do_action( 'smartocs/after_plugin_' . $plugin_slug . '_settings_imported', $settings );
		...
		if ( $single_option_name ) {
			...
			$option_value = Helpers::apply_filters( 'smartocs/import_plugin_option_value', $option_value, $single_option_name, $plugin_slug );
			$option_value = Helpers::apply_filters( 'smartocs/import_plugin_' . $plugin_slug . '_option_value', $option_value, $single_option_name );
			...
			Helpers::do_action( 'smartocs/after_plugin_option_imported', $single_option_name, $option_value, $old_value, $plugin_slug );
			Helpers::do_action( 'smartocs/after_plugin_' . $plugin_slug . '_option_imported', $single_option_name, $option_value, $old_value );
		...
		foreach ( $settings as $option_name => $option_value ) {
			...
			$option_value = Helpers::apply_filters( 'smartocs/import_plugin_option_value', $option_value, $option_name, $plugin_slug );
			$option_value = Helpers::apply_filters( 'smartocs/import_plugin_' . $plugin_slug . '_option_value', $option_value, $option_name );
			...
			if ( $updated || false === $old_value ) {
				...
				Helpers::do_action( 'smartocs/after_plugin_option_imported', $option_name, $option_value, $old_value, $plugin_slug );
				Helpers::do_action( 'smartocs/after_plugin_' . $plugin_slug . '_option_imported', $option_name, $option_value, $old_value );
			}
		}
	...
	private static function detect_single_option_plugin( $plugin_slug, $settings ) {
		...
		if ( count( $settings ) === 1 ) {
			...
			if ( preg_match( '/_(options|settings|config)$/i', $single_key ) && is_array( $single_value ) && ! empty( $single_value ) ) {
				...
				if ( $has_nested_structure || $has_fields_structure || $has_name_icon_structure ) {
					...
					$detected_option = Helpers::apply_filters( 'smartocs/detect_single_option_plugin', $single_key, $plugin_slug, $settings );
		...
		if ( $has_fields_structure || $has_name_icon_structure || ( $has_nested_structure && count( $settings ) > 2 ) ) {
			...
			$detected_option = Helpers::apply_filters( 'smartocs/detect_single_option_plugin', false, $plugin_slug, $settings );
			if ( is_string( $detected_option ) && ! empty( $detected_option ) ) {
				return $detected_option;
			}
			...
			foreach ( $possible_option_names as $option_name ) {
				$is_correct_option = Helpers::apply_filters( 'smartocs/is_single_option_plugin', false, $plugin_slug, $option_name, $settings );
	...
	private static function format_results_for_log( $results ) {
		if ( empty( $results ) ) {
			return esc_html__( 'No results for plugin settings import!', 'smart-one-click-setup' );
		}
		...
		if ( isset( $results['success'] ) || isset( $results['failed'] ) || isset( $results['skipped'] ) ) {
			...
			$output .= esc_html( sprintf( __( 'Plugins imported: %d', 'smart-one-click-setup' ), absint( $results['success'] ) ) ) . PHP_EOL;
			/* translators: %d: Number of plugins */
			$output .= esc_html( sprintf( __( 'Plugins failed: %d', 'smart-one-click-setup' ), absint( $results['failed'] ) ) ) . PHP_EOL;
			/* translators: %d: Number of plugins */
			$output .= esc_html( sprintf( __( 'Plugins skipped: %d', 'smart-one-click-setup' ), absint( $results['skipped'] ) ) ) . PHP_EOL;
			$output .= PHP_EOL;
			...
			if ( ! empty( $results['details'] ) ) {
				foreach ( $results['details'] as $detail ) {
					...
					$output .= esc_html( $status_icon ) . ' ' . esc_html( $detail['plugin'] ) . ' - ' . esc_html( $detail['message'] ) . PHP_EOL;
					...
					if ( 'success' === $detail['status'] && ! empty( $results['options'][ $detail['plugin'] ] ) ) {
						...
						if ( defined( 'WP_DEBUG' ) && WP_DEBUG && ! empty( $options ) ) {
							$output .= '  ' . esc_html__( 'Imported options:', 'smart-one-click-setup' ) . ' ' . esc_html( implode( ', ', array_slice( $options, 0, 10 ) ) );
							if ( count( $options ) > 10 ) {
								/* translators: %d: Number of additional options */
								$output .= ' ' . sprintf( esc_html__( 'and %d more...', 'smart-one-click-setup' ), count( $options ) - 10 );
							}
							$output .= PHP_EOL;
						}
					}
