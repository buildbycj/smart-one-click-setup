<?php
/**
 * Custom WXR Importer class - does not extend WP_Importer
 * Self-contained importer that uses WordPress core functions directly
 *
 * @package smartocs
 */
...
/**
 * Custom WXR Importer class
 * Implements WordPress XML import functionality without extending WP_Importer
 */
class CustomWXRImporter {
	/**
	 ...
	 * Logger instance.
	 *
	 * @var \BuildByCj\WPContentImporter2\WPImporterLogger
	 */
	protected $logger;
	...
	protected function get_reader( $file ) {
		...
		if ( ! $status ) {
			return new WP_Error( 'wxr_importer.cannot_parse', __( 'Could not open the file for parsing', 'smart-one-click-setup' ) );
		}
	 ...
	 *
	 * @return \BuildByCj\WPContentImporter2\WXRImportInfo|WP_Error
	 */
	public function get_preliminary_information( $file ) {
		...
		// Start parsing!
		$data = new \BuildByCj\WPContentImporter2\WXRImportInfo();
		while ( $reader->read() ) {
				...
				case 'wp:wxr_version':
					...
					if ( version_compare( $this->version, self::MAX_WXR_VERSION, '>' ) ) {
						$this->logger->warning(
							...
							sprintf( /* translators: %1$s: WXR file version, %2$s: Maximum supported version */
								__( 'This WXR file (version %1$s) is newer than the importer (version %2$s) and may not be supported. Please consider updating.', 'smart-one-click-setup' ),
							$this->version,
	...
	protected function import_start( $file ) {
		if ( ! is_file( $file ) ) {
			return new WP_Error( 'wxr_importer.file_missing', __( 'The file does not exist, please try again.', 'smart-one-click-setup' ) );
		}
		 ...
		 */
		do_action( 'smartocs_import_start' );
	}
	...
	public function set_user_mapping( $mapping ) {
		foreach ( $mapping as $map ) {
			if ( empty( $map['old_slug'] ) || empty( $map['old_id'] ) || empty( $map['new_id'] ) ) {
				$this->logger->warning( __( 'Invalid author mapping', 'smart-one-click-setup' ) );
				// phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_var_export
			...
			switch ( $child->tagName ) {
				...
				case 'wp:status':
					...
					if ( $data['post_status'] === 'auto-draft' ) {
						...
						return new WP_Error( // Bail now
							'wxr_importer.post.cannot_import_draft',
							__( 'Cannot import auto-draft posts', 'smart-one-click-setup' ),
							$data
						);
					}
					break;
	...
	protected function process_post( $data, $meta, $comments, $terms ) {
		 ...
		 * @param array $terms Terms on the post.
		 */
		$data = apply_filters( 'smartocs_wxr_importer.pre_process.post', $data, $meta, $comments, $terms );
		if ( empty( $data ) ) {
			return false;
		}
		...
		if ( ! $post_type_object ) {
			$this->logger->warning(
				sprintf( /* translators: %1$s: Post title, %2$s: Post type */
					__( 'Failed to import "%1$s": Invalid post type %2$s', 'smart-one-click-setup' ),
				$data['post_title'],
				$data['post_type']
			) );
			return false;
		...
		if ( $post_exists ) {
			$this->logger->info(
				sprintf( /* translators: %1$s: Post type singular name, %2$s: Post title */
					__( '%1$s "%2$s" already exists.', 'smart-one-click-setup' ),
		...
		if ( 'attachment' === $postdata['post_type'] ) {
			if ( ! $this->options['fetch_attachments'] ) {
				$this->logger->notice(
					...
					sprintf( /* translators: %s: Attachment title */
						__( 'Skipping attachment "%s", fetching attachments disabled', 'smart-one-click-setup' ),
					$data['post_title']
				) );
				return false;
			}
		...
		if ( is_wp_error( $post_id ) ) {
			$this->logger->error(
				sprintf( /* translators: %1$s: Post title, %2$s: Post type singular name */
					__( 'Failed to import "%1$s" (%2$s)', 'smart-one-click-setup' ),
				$data['post_title'],
				$post_type_object->labels->singular_name
			) );
			 ...
			 */
			do_action( 'smartocs_wxr_importer.process_failed.post', $post_id, $data, $meta, $comments, $terms );
			return false;
		}
		...
		$this->logger->info(
			sprintf( /* translators: %1$s: Post title, %2$s: Post type singular name */
				__( 'Imported "%1$s" (%2$s)', 'smart-one-click-setup' ),
				$data['post_title'],
				$post_type_object->labels->singular_name
			)
		);
		$this->logger->debug(
			sprintf( /* translators: %1$d: Original post ID, %2$d: New post ID */
				__( 'Post %1$d remapped to %2$d', 'smart-one-click-setup' ),
			$original_id,
			$post_id
		) );
					...
					if ( 'post_format' === $taxonomy ) {
						...
						if ( empty( $term_id ) ) {
							...
							if ( ! is_wp_error( $t ) ) {
								...
								$this->logger->warning(
									sprintf( /* translators: %1$s: Taxonomy name, %2$s: Term name */
										esc_html__( 'Failed to import term: %1$s - %2$s', 'smart-one-click-setup' ),
									esc_html( $taxonomy ),
									esc_html( $term['name'] )
								) );
								continue;
		 ...
		 * @param array $terms Raw term data, already processed.
		 */
		do_action( 'smartocs_wxr_importer.processed.post', $post_id, $data, $meta, $comments, $terms );
	}
	...
	protected function process_attachment( $post, $meta, $remote_url ) {
		...
		if ( ! $info ) {
			return new WP_Error( 'attachment_processing_error', __( 'Invalid file type', 'smart-one-click-setup' ) );
		}
	...
	protected function process_post_meta( $meta, $post_id, $post ) {
		...
		foreach ( $meta as $meta_item ) {
			 ...
			 */
			$meta_item = apply_filters( 'smartocs_wxr_importer.pre_process.post_meta', $meta_item, $post_id );
			if ( empty( $meta_item ) ) {
				return false;
			}

			$key = apply_filters( 'smartocs_import_post_meta_key', $meta_item['key'], $post_id, $post );
			$value = false;
			...
			if ( $key ) {
				...
				do_action( 'smartocs_import_post_meta', $post_id, $key, $value );
	...
	protected function process_comments( $comments, $post_id, $post, $post_exists = false ) {
		...
		foreach ( $comments as $key => $comment ) {
			 ...
			 */
			$comment = apply_filters( 'smartocs_wxr_importer.pre_process.comment', $comment, $post_id );
			if ( empty( $comment ) ) {
				return false;
			}
			 ...
			 */
			do_action( 'smartocs_wxr_importer.processed.comment', $comment_id, $comment, $meta, $post_id );
	...
	protected function process_author( $data, $meta ) {
		 ...
		 * @param array $meta Meta data.
		 */
		$data = apply_filters( 'smartocs_wxr_importer.pre_process.user', $data, $meta );
		if ( empty( $data ) ) {
			return false;
		}
		...
		// Check if user already exists by login or email before attempting to insert.
		...
		// If not found by login, check by email.
		if ( ! $existing_user_id && ! empty( $userdata['user_email'] ) ) {
			$existing_user = get_user_by( 'email', $userdata['user_email'] );
			if ( $existing_user ) {
				$existing_user_id = $existing_user->ID;
				// Update the login to match the existing user for mapping.
				$userdata['user_login'] = $existing_user->user_login;
			}
		}

		// If user exists, map it instead of creating a new one.
		if ( $existing_user_id ) {
			...
			$this->logger->info(
				sprintf( /* translators: %1$s: User login name, %2$d: User ID */
					__( 'User "%1$s" already exists, mapped to existing user ID %2$d', 'smart-one-click-setup' ),
					$userdata['user_login'],
					$user_id
				)
			);
			...
			/**
			 * User processing completed (existing user mapped).
			 *
			 * @param int $user_id Existing user ID.
			 * @param array $userdata Raw data imported for the user.
			 */
			do_action( 'smartocs_wxr_importer.processed.user', $user_id, $userdata );
			return false;
		}

		$user_id = wp_insert_user( wp_slash( $userdata ) );
		if ( is_wp_error( $user_id ) ) {
			// Check if the error is due to existing user (race condition or case sensitivity).
			$error_code = $user_id->get_error_code();
			if ( 'existing_user_login' === $error_code || 'existing_user_email' === $error_code ) {
			...
			$this->logger->error(
				sprintf( /* translators: %1$s: User login name, %2$s: Error message */
					__( 'Failed to import user "%1$s": %2$s', 'smart-one-click-setup' ),
					$userdata['user_login'],
			 ...
			 * @param array $userdata Raw data imported for the user.
			 */
			do_action( 'smartocs_wxr_importer.process_failed.user', $user_id, $userdata );
			return false;
		}
		...
		$this->logger->info(
			sprintf( /* translators: %s: User login name */
				__( 'Imported user "%s"', 'smart-one-click-setup' ),
				$userdata['user_login']
			)
		);
		$this->logger->debug(
			sprintf( /* translators: %1$d: Original user ID, %2$d: New user ID */
				__( 'User %1$d remapped to %2$d', 'smart-one-click-setup' ),
			$original_id,
			$user_id
		) );
	...
	protected function process_term( $data, $meta ) {
		 ...
		 * @param array $meta Meta data.
		 */
		$data = apply_filters( 'smartocs_wxr_importer.pre_process.term', $data, $meta );
		if ( empty( $data ) ) {
			return false;
		}
		...
		if ( is_wp_error( $result ) ) {
			 ...
			 */
			do_action( 'smartocs_wxr_importer.process_failed.term', $result, $data, $meta );
			return false;
		}
		...
		$this->logger->info(
			sprintf( /* translators: %1$s: Term name, %2$s: Taxonomy name */
				__( 'Imported "%1$s" (%2$s)', 'smart-one-click-setup' ),
				$data['name'],
				$data['taxonomy']
			)
		...
		$this->logger->debug(
			sprintf( /* translators: %1$d: Original term ID, %2$d: New term ID */
				__( 'Term %1$d remapped to %2$d', 'smart-one-click-setup' ),
			$original_id,
			$term_id
		) );
		 ...
		 * @param array $data Raw data imported for the term.
		 */
		do_action( 'smartocs_wxr_importer.processed.term', $term_id, $data );
	}
	...
	protected function process_term_meta( $meta, $term_id, $term ) {
		...
		foreach ( $meta as $meta_item ) {
			 ...
			 */
			$meta_item = apply_filters( 'smartocs_wxr_importer.pre_process.term_meta', $meta_item, $term_id );

			if ( empty( $meta_item ) ) {
				continue;
			}

			$key = apply_filters( 'smartocs_import_term_meta_key', $meta_item['key'], $term_id, $term );
			$value = false;
			...
			if ( $key ) {
				...
				if ( is_wp_error( $result ) ) {
					$this->logger->warning(
						sprintf( /* translators: %1$s: Meta key, %2$s: Meta value, %3$d: Term ID */
							__( 'Failed to add metakey: %1$s, metavalue: %2$s to term_id: %3$d', 'smart-one-click-setup' ),
						$key,
						$log_value,
						$term_id
					) );
					do_action( 'smartocs_wxr_importer.process_failed.termmeta', $result, $meta_item, $term_id, $term );
				}
				else {
					$this->logger->debug(
						sprintf( /* translators: %1$d: Term ID, %2$s: Meta key, %3$s: Meta value */
							__( 'Meta for term_id %1$d : %2$s => %3$s ; successfully added!', 'smart-one-click-setup' ),
							$term_id,
							$key,
							$log_value
						)
				...
				do_action( 'smartocs_import_term_meta', $term_id, $key, $value );
			}
		}
	...
	protected function fetch_remote_file( $url, $post ) {
		...
		if ( $code !== 200 ) {
			...
			return new WP_Error(
				'import_file_error',
				sprintf( /* translators: %1$d: HTTP status code, %2$s: Status description, %3$s: URL */
					__( 'Remote server returned %1$d %2$s for %3$s', 'smart-one-click-setup' ),
					$code,
		...
		// 	wp_delete_file( $upload['file'] );
		// 	return new WP_Error( 'import_file_error', __( 'Remote file is incorrect size', 'smart-one-click-setup' ) );
		// }

		if ( 0 === $filesize ) {
			wp_delete_file( $upload['file'] );
			return new WP_Error( 'import_file_error', __( 'Zero size file downloaded', 'smart-one-click-setup' ) );
		}

		$max_size = (int) $this->max_attachment_size();
		if ( ! empty( $max_size ) && $filesize > $max_size ) {
			wp_delete_file( $upload['file'] );
			$message = sprintf( /* translators: %s: Maximum file size */
				__( 'Remote file is too large, limit is %s', 'smart-one-click-setup' ),
		...
		foreach ( $todo as $post_id => $_ ) {
			...
			if ( ! empty( $parent_id ) ) {
				...
				if ( isset( $this->mapping['post'][ $parent_id ] ) ) {
					...
					$this->logger->warning(
						sprintf( /* translators: %1$s: Post title, %2$d: Post ID */
							__( 'Could not find the post parent for "%1$s" (post #%2$d)', 'smart-one-click-setup' ),
							get_the_title( $post_id ),
							$post_id
						)
					);
					$this->logger->debug(
						sprintf( /* translators: %1$d: Post ID, %2$d: Parent post ID */
							__( 'Post %1$d was imported with parent %2$d, but could not be found', 'smart-one-click-setup' ),
						$post_id,
						$parent_id
					) );
				}
			...
			if ( ! empty( $author_slug ) ) {
				...
				if ( isset( $this->mapping['user_slug'][ $author_slug ] ) ) {
					...
					$this->logger->warning(
						sprintf( /* translators: %1$s: Post title, %2$d: Post ID */
							__( 'Could not find the author for "%1$s" (post #%2$d)', 'smart-one-click-setup' ),
							get_the_title( $post_id ),
							$post_id
						)
					);
					$this->logger->debug(
						sprintf( /* translators: %1$d: Post ID, %2$s: Author slug */
							__( 'Post %1$d was imported with author "%2$s", but could not be found', 'smart-one-click-setup' ),
						$post_id,
						$author_slug
					) );
				}
			...
			if ( empty( $data ) ) {
				$this->logger->debug(
					sprintf( /* translators: %d: Post ID */
						__( 'Post %d was marked for post-processing, but none was required.', 'smart-one-click-setup' ),
					$post_id
				) );
				continue;
			}
			...
			if ( is_wp_error( $result ) ) {
				$this->logger->warning(
					sprintf( /* translators: %1$s: Post title, %2$d: Post ID */
						__( 'Could not update "%1$s" (post #%2$d) with mapped data', 'smart-one-click-setup' ),
					get_the_title( $post_id ),
					$post_id
				) );
				$this->logger->debug( $result->get_error_message() );
	...
	protected function post_process_menu_item( $post_id ) {
		...
		if ( ! empty( $menu_object ) ) {
			...
			$this->logger->warning(
				sprintf( /* translators: %1$s: Post title, %2$d: Post ID */
					__( 'Could not find the menu object for "%1$s" (post #%2$d)', 'smart-one-click-setup' ),
					get_the_title( $post_id ),
					$post_id
				)
			);
			$this->logger->debug(
				sprintf( /* translators: %1$d: Post ID, %2$d: Menu object ID, %3$s: Menu item type */
					__( 'Post %1$d was imported with object "%2$d" of type "%3$s", but could not be found', 'smart-one-click-setup' ),
				$post_id,
				$menu_object_id,
				$menu_item_type
			) );
		...
		foreach ( $todo as $comment_id => $_ ) {
				...
				if ( isset( $this->mapping['comment'][ $parent_id ] ) ) {
					...
					$this->logger->warning(
						sprintf( /* translators: %d: Comment ID */
							__( 'Could not find the comment parent for comment #%d', 'smart-one-click-setup' ),
							$comment_id
						)
					);
					$this->logger->debug(
						sprintf( /* translators: %1$d: Comment ID, %2$d: Parent comment ID */
							__( 'Comment %1$d was imported with parent %2$d, but could not be found', 'smart-one-click-setup' ),
						$comment_id,
						$parent_id
					) );
				}
			...
			if ( ! empty( $author_id ) ) {
				...
				if ( isset( $this->mapping['user'][ $author_id ] ) ) {
					...
					$this->logger->warning(
						sprintf( /* translators: %d: Comment ID */
							__( 'Could not find the author for comment #%d', 'smart-one-click-setup' ),
							$comment_id
						)
					);
					$this->logger->debug(
						sprintf( /* translators: %1$d: Comment ID, %2$d: Author user ID */
							__( 'Comment %1$d was imported with author %2$d, but could not be found', 'smart-one-click-setup' ),
						$comment_id,
						$author_id
					) );
				}
			...
			if ( empty( $result ) ) {
				$this->logger->warning(
					sprintf( /* translators: %d: Comment ID */
						__( 'Could not update comment #%d with mapped data', 'smart-one-click-setup' ),
					$comment_id
				) );
				continue;
			}
	...
	protected function post_process_terms( $terms_to_be_remapped ) {
		...
		foreach ( $terms_to_be_remapped as $termid => $term_taxonomy ) {
			...
			if( empty( $termid ) || ! is_numeric( $termid ) ) {
				$this->logger->warning(
					sprintf( /* translators: %s: Term ID */
						__( 'Faulty term_id provided in terms-to-be-remapped array %s', 'smart-one-click-setup' ),
					$termid
					) );
				continue;
			}
			...
			if( empty( $term_taxonomy ) ){
				$this->logger->warning(
					sprintf( /* translators: %d: Term ID */
						__( 'No taxonomy provided in terms-to-be-remapped array for term #%d', 'smart-one-click-setup' ),
					$term_id
					) );
				continue;
			}
			...
			if ( empty( $parent_slug ) ) {
				$this->logger->warning(
					sprintf( /* translators: %d: Term ID */
						__( 'No parent_slug identified in remapping-array for term: %d', 'smart-one-click-setup' ),
					$term_id
					) );
				continue;
			}
			...
			if ( ! isset( $this->mapping['term_slug'][ $parent_slug ] ) || ! is_numeric( $this->mapping['term_slug'][ $parent_slug ] ) ) {
				$this->logger->warning(
					sprintf( /* translators: %1$d: Term ID, %2$s: Parent slug */
						__( 'The term(%1$d)"s parent_slug (%2$s) is not found in the remapping-array.', 'smart-one-click-setup' ),
					$term_id,
					$parent_slug
					) );
				continue;
			...
			if ( empty( $termattributes ) ) {
				$this->logger->warning(
					sprintf( /* translators: %d: Term ID */
						__( 'No data returned by get_term_by for term_id #%d', 'smart-one-click-setup' ),
					$term_id
					) );
				continue;
			}
			...
			if ( is_wp_error( $result ) ) {
				$this->logger->warning(
					sprintf( /* translators: %1$s: Term name, %2$d: Term ID */
						__( 'Could not update "%1$s" (term #%2$d) with mapped data', 'smart-one-click-setup' ),
					$termattributes['name'],
					$term_id
				) );
				$this->logger->debug( $result->get_error_message() );
			...
			delete_term_meta( $term_id, '_wxr_import_parent' );
			$this->logger->debug(
				sprintf( /* translators: %1$d: Term ID, %2$d: Parent term ID */
					__( 'Term %1$d was successfully updated with parent %2$d', 'smart-one-click-setup' ),
				$term_id,
				$mapped_parent
			) );
		}
	...
	function remap_featured_images() {
		...
		$this->logger->info( esc_html__( 'Starting remapping of featured images', 'smart-one-click-setup' ) );
		...
		foreach ( $this->featured_images as $post_id => $value ) {
			if ( isset( $this->mapping['post'][ $value ] ) ) {
				...
				if ( $new_id !== $value ) {
					$this->logger->info(
						sprintf( /* translators: %1$d: Original image ID, %2$d: New image ID, %3$d: Post ID */
							esc_html__( 'Remapping featured image ID %1$d to new ID %2$d for post ID %3$d', 'smart-one-click-setup' ),
							$value,
							$new_id,
							$post_id
						)
	 ...
	 * Default is 0 (unlimited), can be filtered via smartocs_import_attachment_size_limit
	 *
	 * @return int Maximum attachment file size to import
	 */
	protected function max_attachment_size() {
		return apply_filters( 'smartocs_import_attachment_size_limit', 0 );
	}
	...
	protected function post_exists( $data ) {
		...
		if ( $this->options['prefill_existing_posts'] ) {
			// SOCS: fix for custom post types. The guids in the prefilled section are escaped, so these ones should be as well.
