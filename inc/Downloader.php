...
class Downloader {
	...
	private function get_content_from_url( $url ) {
		...
		$response = wp_remote_get(
			$url,
			array( 'timeout' => Helpers::apply_filters( 'socs/timeout_for_downloading_import_file', 20 ) )
		);
		...
		if ( is_wp_error( $response ) || 200 !== $response['response']['code'] ) {
			...
			return new \WP_Error(
				...
				Helpers::apply_filters( 'socs/message_after_file_fetching_error', '' )
			);
		}
	...
	public function set_download_directory_path( $download_directory_path ) {
		if ( file_exists( $download_directory_path ) ) {
			...
			$this->download_directory_path = Helpers::apply_filters( 'socs/upload_file_path', trailingslashit( $upload_dir['path'] ) );
		}
	}
}
