<?php
/**
 * Static functions used in the SOCS plugin.
 *
 * @package socs
 */
...
class Helpers {
	...
	public static function validate_import_file_info( $import_files ) {
		$socs_filtered_import_file_info = array();

		foreach ( $import_files as $import_file ) {
			if ( self::is_import_file_info_format_correct( $import_file ) ) {
				$socs_filtered_import_file_info[] = $import_file;
			}
		}

		return $socs_filtered_import_file_info;
	}
	...
	public static function download_import_files( $import_file_info ) {
		...
		$socs_downloader = new Downloader();

		$import_file_info = self::apply_filters('socs/pre_download_import_files', $import_file_info);
		...
		if ( empty( $import_file_info['import_file_url'] ) ) {
			if ( ! empty( $import_file_info['local_import_file'] ) && file_exists( $import_file_info['local_import_file'] ) ) {
				$socs_downloaded_files['content'] = $import_file_info['local_import_file'];
			}
		}
		else {
			// Set the filename string for content import file.
			$socs_content_filename = self::apply_filters( 'socs/downloaded_content_file_prefix', 'demo-content-import-file_' ) . self::$demo_import_start_time . self::apply_filters( 'socs/downloaded_content_file_suffix_and_file_extension', '.xml' );

			// Download the content import file.
			$socs_downloaded_files['content'] = $socs_downloader->download_file( $import_file_info['import_file_url'], $socs_content_filename );

			// Return from this function if there was an error.
			if ( is_wp_error( $socs_downloaded_files['content'] ) ) {
				return $socs_downloaded_files['content'];
			}
		}
		...
		if ( ! empty( $import_file_info['import_widget_file_url'] ) ) {
			...
			$socs_widget_filename = self::apply_filters( 'socs/downloaded_widgets_file_prefix', 'demo-widgets-import-file_' ) . self::$demo_import_start_time . self::apply_filters( 'socs/downloaded_widgets_file_suffix_and_file_extension', '.json' );

			// Download the widgets import file.
			$socs_downloaded_files['widgets'] = $socs_downloader->download_file( $import_file_info['import_widget_file_url'], $socs_widget_filename );

			// Return from this function if there was an error.
			if ( is_wp_error( $socs_downloaded_files['widgets'] ) ) {
				return $socs_downloaded_files['widgets'];
			}
		}
		else if ( ! empty( $import_file_info['local_import_widget_file'] ) ) {
			if ( file_exists( $import_file_info['local_import_widget_file'] ) ) {
				$socs_downloaded_files['widgets'] = $import_file_info['local_import_widget_file'];
			}
		}
		...
		if ( ! empty( $import_file_info['import_customizer_file_url'] ) ) {
			...
			$socs_customizer_filename = self::apply_filters( 'socs/downloaded_customizer_file_prefix', 'demo-customizer-import-file_' ) . self::$demo_import_start_time . self::apply_filters( 'socs/downloaded_customizer_file_suffix_and_file_extension', '.dat' );

			// Download the customizer import file.
			$socs_downloaded_files['customizer'] = $socs_downloader->download_file( $import_file_info['import_customizer_file_url'], $socs_customizer_filename );

			// Return from this function if there was an error.
			if ( is_wp_error( $socs_downloaded_files['customizer'] ) ) {
				return $socs_downloaded_files['customizer'];
			}
		}
		else if ( ! empty( $import_file_info['local_import_customizer_file'] ) ) {
			if ( file_exists( $import_file_info['local_import_customizer_file'] ) ) {
				$socs_downloaded_files['customizer'] = $import_file_info['local_import_customizer_file'];
			}
		}
		...
		if ( ! empty( $import_file_info['import_redux'] ) && is_array( $import_file_info['import_redux'] ) ) {
			$socs_redux_items = array();

			// Setup filename paths to save the Redux content.
			foreach ( $import_file_info['import_redux'] as $index => $redux_item ) {
				$socs_redux_filename = self::apply_filters( 'socs/downloaded_redux_file_prefix', 'demo-redux-import-file_' ) . $index . '-' . self::$demo_import_start_time . self::apply_filters( 'socs/downloaded_redux_file_suffix_and_file_extension', '.json' );

				// Download the Redux import file.
				$socs_file_path = $socs_downloader->download_file( $redux_item['file_url'], $socs_redux_filename );

				// Return from this function if there was an error.
				if ( is_wp_error( $socs_file_path ) ) {
					return $socs_file_path;
				}

				$socs_redux_items[] = array(
					'option_name' => $redux_item['option_name'],
					'file_path'   => $socs_file_path,
				);
			}

			// Download the Redux import file.
			$socs_downloaded_files['redux'] = $socs_redux_items;
		}
		...
		if ( ! empty( $import_file_info['import_wpforms_file_url'] ) ) {
			...
			$socs_wpforms_filename = self::apply_filters( 'socs/downloaded_wpforms_file_prefix', 'demo-wpforms-import-file_' ) . self::$demo_import_start_time . self::apply_filters( 'socs/downloaded_wpforms_file_suffix_and_file_extension', '.json' );

			// Download the customizer import file.
			$socs_downloaded_files['wpforms'] = $socs_downloader->download_file( $import_file_info['import_wpforms_file_url'], $socs_wpforms_filename );

			// Return from this function if there was an error.
			if ( is_wp_error( $socs_downloaded_files['wpforms'] ) ) {
				return $socs_downloaded_files['wpforms'];
			}
		} else if ( ! empty( $import_file_info['local_import_wpforms_file'] ) ) {
			if ( file_exists( $import_file_info['local_import_wpforms_file'] ) ) {
				$socs_downloaded_files['wpforms'] = $import_file_info['local_import_wpforms_file'];
			}
		}

		return $socs_downloaded_files;
	}
	...
	public static function write_to_file( $content, $file_path ) {
		...
		$socs_verified_credentials = self::check_wp_filesystem_credentials();

		if ( is_wp_error( $socs_verified_credentials ) ) {
			return $socs_verified_credentials;
		}
	...
	public static function append_to_file( $content, $file_path, $separator_text = '' ) {
		...
		if ( file_exists( $file_path ) ) {
			$socs_existing_data = $wp_filesystem->get_contents( $file_path );
		}

		// Style separator.
		$socs_separator = PHP_EOL . '---' . $separator_text . '---' . PHP_EOL;

		if ( ! $wp_filesystem->put_contents( $file_path, $socs_existing_data . $socs_separator . $content . PHP_EOL ) ) {
			return new \WP_Error(
				'failed_writing_file_to_server',
				sprintf( /* translators: %1$s - br HTML tag, %2$s - file path */
					__( 'An error occurred while writing file to your server! Tried to write a file to: %1$s%2$s.', 'smart-one-click-setup' ),
	...
	public static function data_from_file( $file_path ) {
		...
		$socs_data = $wp_filesystem->get_contents( $file_path );
	...
	private static function check_wp_filesystem_credentials() {
		...
		if ( ! ( 'direct' === get_filesystem_method() ) ) {
			return new \WP_Error(
				...
				sprintf( /* translators: %1$s and %2$s - strong HTML tags, %3$s - HTML link to a doc page. */
					__( 'This WordPress page does not have %1$sdirect%2$s write file access. This plugin needs it in order to save the demo import xml file to the upload directory of your site. You can change this setting with these instructions: %3$s.', 'smart-one-click-setup' ),
					'<strong>',
					'</strong>',
		...
		$socs_plugin_page_setup = self::get_plugin_page_setup_data();

		// Get user credentials for WP file-system API.
		$socs_demo_import_page_url = wp_nonce_url( $socs_plugin_page_setup['parent_slug'] . '?page=' . $socs_plugin_page_setup['menu_slug'], $socs_plugin_page_setup['menu_slug'] );

		if ( false === ( $socs_creds = request_filesystem_credentials( $socs_demo_import_page_url, '', false, false, null ) ) ) {
			return new \WP_error(
				'filesystem_credentials_could_not_be_retrieved',
				__( 'An error occurred while retrieving reading/writing permissions to your server (could not retrieve WP filesystem credentials)!', 'smart-one-click-setup' )
			);
		}

		// Now we have credentials, try to get the wp_filesystem running.
		if ( ! WP_Filesystem( $socs_creds ) ) {
			return new \WP_Error(
				'wrong_login_credentials',
				__( 'Your WordPress login credentials don\'t allow to use WP_Filesystem!', 'smart-one-click-setup' )
			);
		}
	...
	public static function get_log_path() {
		$socs_upload_dir  = wp_upload_dir();
		$socs_upload_path = self::apply_filters( 'socs/upload_file_path', trailingslashit( $socs_upload_dir['path'] ) );

		$socs_log_path = $socs_upload_path . self::apply_filters( 'socs/log_file_prefix', 'log_file_' ) . self::$demo_import_start_time . self::apply_filters( 'socs/log_file_suffix_and_file_extension', '.txt' );

		self::register_file_as_media_attachment( $socs_log_path );

		return $socs_log_path;
	}
	...
	public static function register_file_as_media_attachment( $log_path ) {
		...
		$socs_log_mimes = array( 'txt' => 'text/plain' );
		$socs_filetype  = wp_check_filetype( basename( $log_path ), self::apply_filters( 'socs/file_mimes', $socs_log_mimes ) );

		// Prepare an array of post data for the attachment.
		$socs_attachment = array(
			'guid'           => self::get_log_url( $log_path ),
			'post_mime_type' => $socs_filetype['type'],
			'post_title'     => self::apply_filters( 'socs/attachment_prefix', esc_html__( 'Smart One Click Setup - ', 'smart-one-click-setup' ) ) . preg_replace( '/\.[^.]+$/', '', basename( $log_path ) ),
			'post_content'   => '',
			'post_status'    => 'inherit',
		);

		// Insert the file as attachment in Media page.
		$socs_attach_id = wp_insert_attachment( $socs_attachment, $log_path );
	}
	...
	public static function get_log_url( $log_path ) {
		$socs_upload_dir = wp_upload_dir();
		$socs_upload_url = self::apply_filters( 'socs/upload_file_url', trailingslashit( $socs_upload_dir['url'] ) );

		return $socs_upload_url . basename( $log_path );
	}
	...
	public static function verify_ajax_call() {
		check_ajax_referer( 'socs-ajax-verification', 'security' );
	...
	public static function process_uploaded_files( $uploaded_files, $log_file_path ) {
		...
		$socs_selected_import_files = array(
			'content'    => '',
			'widgets'    => '',
			'customizer' => '',
			'redux'      => '',
		...
		add_filter( 'upload_mimes', function ( $defaults ) {
			...
			return array_merge( $socs_custom, $defaults );
		} );

		// Error data if the demo file was not provided.
		$socs_file_not_provided_error = array(
		...
		// phpcs:ignore WordPress.Security.NonceVerification.Missing -- Nonce verified in calling AJAX callback.
		$socs_content_file_info = isset( $_FILES['content_file'] ) ?
			// phpcs:ignore WordPress.Security.NonceVerification.Missing, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Nonce verified in calling AJAX callback.
			wp_handle_upload( $_FILES['content_file'], $socs_upload_overrides ) :
			$socs_file_not_provided_error;

		// phpcs:ignore WordPress.Security.NonceVerification.Missing -- Nonce verified in calling AJAX callback.
		$socs_widget_file_info = isset( $_FILES['widget_file'] ) ?
			// phpcs:ignore WordPress.Security.NonceVerification.Missing, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Nonce verified in calling AJAX callback.
			wp_handle_upload( $_FILES['widget_file'], $socs_upload_overrides ) :
			$socs_file_not_provided_error;

		// phpcs:ignore WordPress.Security.NonceVerification.Missing -- Nonce verified in calling AJAX callback.
		$socs_customizer_file_info = isset( $_FILES['customizer_file'] ) ?
			// phpcs:ignore WordPress.Security.NonceVerification.Missing, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Nonce verified in calling AJAX callback.
			wp_handle_upload( $_FILES['customizer_file'], $socs_upload_overrides ) :
			$socs_file_not_provided_error;

		// phpcs:ignore WordPress.Security.NonceVerification.Missing -- Nonce verified in calling AJAX callback.
		$socs_redux_file_info = isset( $_FILES['redux_file'] ) ?
			// phpcs:ignore WordPress.Security.NonceVerification.Missing, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Nonce verified in calling AJAX callback.
			wp_handle_upload( $_FILES['redux_file'], $socs_upload_overrides ) :
			$socs_file_not_provided_error;

		// Process content import file.
		if ( $socs_content_file_info && ! isset( $socs_content_file_info['error'] ) ) {
			// Set uploaded content file.
			$socs_selected_import_files['content'] = $socs_content_file_info['file'];
		}
		else {
			// Add this error to log file.
			$socs_log_added = self::append_to_file(
				sprintf( /* translators: %s - the error message. */
					__( 'Content file was not uploaded. Error: %s', 'smart-one-click-setup' ),
					$socs_content_file_info['error']
				),
				$log_file_path,
		...
		if ( $socs_widget_file_info && ! isset( $socs_widget_file_info['error'] ) ) {
			// Set uploaded widget file.
			$socs_selected_import_files['widgets'] = $socs_widget_file_info['file'];
		}
		else {
			// Add this error to log file.
			$socs_log_added = self::append_to_file(
		...
		if ( $socs_customizer_file_info && ! isset( $socs_customizer_file_info['error'] ) ) {
			// Set uploaded customizer file.
			$socs_selected_import_files['customizer'] = $socs_customizer_file_info['file'];
		}
		else {
			// Add this error to log file.
			$socs_log_added = self::append_to_file(
				sprintf( /* translators: %s - the error message. */
					...
					$socs_customizer_file_info['error']
				),
				$log_file_path,
				esc_html__( 'Upload files' , 'smart-one-click-setup' )
			);
		...
		if ( $socs_redux_file_info && ! isset( $socs_redux_file_info['error'] ) ) {
			...
			$socs_selected_import_files['redux'] = array(
				array(
					// phpcs:ignore WordPress.Security.NonceVerification.Missing
					'option_name' => sanitize_text_field( wp_unslash( $_POST['redux_option_name'] ) ),
					'file_path'   => $socs_redux_file_info['file'],
				),
			);
		}
		else {
		...
		$socs_log_added = self::append_to_file(
			__( 'The import files were successfully uploaded!', 'smart-one-click-setup' ) . self::import_file_info( $socs_selected_import_files ),
			$log_file_path,
			esc_html__( 'Upload files' , 'smart-one-click-setup' )
		);

		// Return array with paths of uploaded files.
		return $socs_selected_import_files;
	}
	...
	public static function import_file_info( $selected_import_files ) {
		$socs_redux_file_string = '';

		if ( ! empty( $selected_import_files['redux'] ) ) {
			$socs_redux_file_string = array_reduce( $selected_import_files['redux'], function( $string, $item ) {
		...
		sprintf( /* translators: %1$s - new line break, %2$s - the site URL, %3$s - the file path for content import, %4$s - the file path for widgets import, %5$s - the file path for widgets import, %6$s - the file path for redux import. */
			...
			empty( $socs_redux_file_string ) ? esc_html__( 'not defined!', 'smart-one-click-setup' ) : $socs_redux_file_string
		);
	}
	...
	public static function set_demo_import_start_time() {
		self::$demo_import_start_time = gmdate( self::apply_filters( 'socs/date_format_for_file_names', 'Y-m-d__H-i-s' ) );
	}
	...
	public static function get_all_demo_import_categories( $demo_imports ) {
		$socs_categories = array();

		foreach ( $demo_imports as $item ) {
			if ( ! empty( $item['categories'] ) && is_array( $item['categories'] ) ) {
				foreach ( $item['categories'] as $category ) {
					$socs_categories[ sanitize_key( $category ) ] = $category;
				}
			}
		}

		if ( empty( $socs_categories ) ) {
			return false;
		}

		return $socs_categories;
	}
	...
	public static function get_demo_import_item_categories( $item ) {
		$socs_sanitized_categories = array();

		if ( isset( $item['categories'] ) ) {
			foreach ( $item['categories'] as $category ) {
				$socs_sanitized_categories[] = sanitize_key( $category );
			}
		}

		if ( ! empty( $socs_sanitized_categories ) ) {
			return implode( ',', $socs_sanitized_categories );
		}
	...
	/**
	 * Set the SOCS transient with the current importer data.
	 *
	 * @param array $data Data to be saved to the transient.
	 */
	public static function set_socs_import_data_transient( $data ) {
		set_transient( 'socs_importer_data', $data, 0.1 * HOUR_IN_SECONDS );
	}


	/**
	 * Backwards compatible apply_filters helper.
	 * With 3.0 we changed the filter prefix from 'pt-socs/' to just 'socs/',
	 ...
	 * With 3.0 we changed the action prefix from 'pt-socs/' to just 'socs/',
	 * but we needed to make sure backwards compatibility is in place.
	...
	public static function get_plugin_page_setup_data() {
		return Helpers::apply_filters( 'socs/plugin_page_setup', array(
	...
	public static function get_failed_attachment_imports() {
		...
		return get_transient( 'socs_importer_data_failed_attachment_imports' );
	}
	...
	public static function set_failed_attachment_import( $attachment_url ) {
		...
		$socs_failed_media_imports = self::get_failed_attachment_imports();

		if ( empty( $socs_failed_media_imports ) || ! is_array( $socs_failed_media_imports ) ) {
			$socs_failed_media_imports = [];
		}

		$socs_failed_media_imports[] = $attachment_url;

		set_transient( 'socs_importer_data_failed_attachment_imports', $socs_failed_media_imports, HOUR_IN_SECONDS );
	}
}
