...
class SmartOneClickSetup {
	...
	/**
	 * The instance of the SMARTOCS\Importer class.
	 *
	 * @var object
	 */
	public $importer;
	...
	/**
	 * Directory where extracted ZIP files are stored (for cleanup after import).
	 *
	 * @var string
	 */
	...
	/**
	 * Track which imports have already been executed to prevent duplicates.
	 *
	 * @var array
	 */
	private $import_execution_flags = array(
		'widgets'          => false,
		'elementor'        => false,
		'redux'            => false,
		'wpforms'          => false,
	...
	protected function __construct() {
		...
		add_action( 'wp_ajax_smartocs_upload_manual_import_files', array( $this, 'upload_manual_import_files_callback' ) );
		add_action( 'wp_ajax_smartocs_import_demo_data', array( $this, 'import_demo_data_ajax_callback' ) );
		add_action( 'wp_ajax_smartocs_import_customizer_data', array( $this, 'import_customizer_data_ajax_callback' ) );
		add_action( 'wp_ajax_smartocs_after_import_data', array( $this, 'after_all_import_data_ajax_callback' ) );
		add_action( 'wp_ajax_smartocs_export_data', array( $this, 'export_data_ajax_callback' ) );
		add_action( 'wp_ajax_smartocs_import_zip_file', array( $this, 'import_zip_file_ajax_callback' ) );
		add_action( 'wp_ajax_smartocs_import_predefined_zip', array( $this, 'import_predefined_zip_ajax_callback' ) );
		...
		add_action( 'smartocs/after_import', [ $this, 'fix_imported_wp_navigation' ] );

		// Initialize static classes to register their dependency loading hooks.
		if ( is_admin() || ( defined( 'WP_CLI' ) && WP_CLI ) ) {
			CustomizerImporter::init();
			PluginSettingsImporter::init();
		}

		// Merge ImportHelper imports with filter-based imports.
		add_filter( 'smartocs/predefined_import_files', array( $this, 'merge_helper_imports' ), 999 );
	}
	...
	public function create_plugin_page() {
		...
		$disable_menu = Helpers::apply_filters( 'smartocs/disable_admin_menu', false );
		...
		$this->plugin_page = add_submenu_page(
			...
			Helpers::apply_filters( 'smartocs/plugin_page_display_callback_function', array( $this, 'display_plugin_page' ) )
		);
		...
		add_submenu_page(
			...
			esc_html__( 'Smart Export', 'smart-one-click-setup' ),
			$this->plugin_page_setup['capability'],
			'smartocs-smart-export',
			Helpers::apply_filters( 'smartocs/export_page_display_callback_function', array( $this, 'display_export_page' ) )
		);
		...
		if ( ! $disable_menu ) {
			register_importer( $this->plugin_page_setup['menu_slug'], $this->plugin_page_setup['page_title'], $this->plugin_page_setup['menu_title'], Helpers::apply_filters( 'smartocs/plugin_page_display_callback_function', array( $this, 'display_plugin_page' ) ) );
		}
	}
	...
	public function display_plugin_page() {
		...
		require_once SMARTOCS_PATH . 'views/smart-import.php';
	}
	...
	public function display_export_page() {
		...
		// 	require_once SMARTOCS_PATH . 'views/export.php';
		// 	return;
		// }
	...
	public function admin_enqueue_scripts( $hook ) {
		...
		$plugin_pages = array(
			$this->plugin_page,
			'appearance_page_smartocs-smart-export',
			'appearance_page_smartocs-smart-import',
		);

		$should_enqueue = in_array( $hook, $plugin_pages, true );

		// Check GET parameter with nonce and capability verification.
		if ( ! $should_enqueue && 'admin.php' === $hook && isset( $_GET['import'] ) ) {
			// Verify nonce and user capability.
			$nonce_valid = false;
			if ( isset( $_GET['_wpnonce'] ) ) {
				$nonce_valid = wp_verify_nonce( sanitize_text_field( wp_unslash( $_GET['_wpnonce'] ) ), 'smartocs-import-action' );
			}
			
			if ( $nonce_valid && current_user_can( 'import' ) && $this->plugin_page_setup['menu_slug'] === sanitize_text_field( wp_unslash( $_GET['import'] ) ) ) {
				$should_enqueue = true;
			}
		}
	...
	public function enqueue_template_scripts( $load_css = true ) {
		wp_enqueue_script( 'smartocs-main-js', SMARTOCS_URL . 'assets/js/main.js' , array( 'jquery' ), SMARTOCS_VERSION, false );

		// Get theme data.
		$theme = wp_get_theme();

		wp_localize_script( 'smartocs-main-js', 'smartocs',
			array(
				'ajax_url'         => admin_url( 'admin-ajax.php' ),
				'ajax_nonce'       => wp_create_nonce( 'smartocs-ajax-verification' ),
				'import_files'     => $this->import_files,
				'wp_customize_on'  => Helpers::apply_filters( 'smartocs/enable_wp_customize_save_hooks', false ),
				'theme_screenshot' => $theme->get_screenshot(),
				'missing_plugins'  => array(),
				'plugin_url'       => SMARTOCS_URL,
		...
		if ( $load_css ) {
			wp_enqueue_style( 'smartocs-main-css', SMARTOCS_URL . 'assets/css/main.css', array() , SMARTOCS_VERSION );
		}
	}
	...
	/**
      ...
      public function upload_manual_import_files_callback() {
            ...
            if ( ! isset( $_POST['security'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['security'] ) ), 'smartocs-ajax-verification' ) ) {
                  wp_send_json_error( esc_html__( 'Security check failed.', 'smart-one-click-setup' ) );
            }
            ...
            // Extract only the specific files needed for processing.
            $specific_files = array();
            $file_keys = array( 'content_file', 'widget_file', 'customizer_file', 'redux_file' );

            // Extract only the specific keys we need from $_FILES and sanitize them.
            $files = array();
            foreach ( $file_keys as $key ) {
                  if ( ! empty( $_FILES[ $key ] ) && is_array( $_FILES[ $key ] ) ) {
                        $files[ $key ] = array(
                              'name'     => isset( $_FILES[ $key ]['name'] ) ? sanitize_file_name( $_FILES[ $key ]['name'] ) : '',
                              'type'     => isset( $_FILES[ $key ]['type'] ) ? sanitize_mime_type( $_FILES[ $key ]['type'] ) : '',
                              'tmp_name' => isset( $_FILES[ $key ]['tmp_name'] ) ? sanitize_text_field( $_FILES[ $key ]['tmp_name'] ) : '',
            ...
            $content_file = $this->get_validated_file_upload( 'content_file', $files );
            if ( false !== $content_file ) {
                  $specific_files['content_file'] = $content_file;
            }
            $widget_file = $this->get_validated_file_upload( 'widget_file', $files );
            if ( false !== $widget_file ) {
                  $specific_files['widget_file'] = $widget_file;
            }
            $customizer_file = $this->get_validated_file_upload( 'customizer_file', $files );
            if ( false !== $customizer_file ) {
                  $specific_files['customizer_file'] = $customizer_file;
            }
            $redux_file = $this->get_validated_file_upload( 'redux_file', $files );
            if ( false !== $redux_file ) {
                  $specific_files['redux_file'] = $redux_file;
            }

            // Get paths for the uploaded files.
            $this->selected_import_files = Helpers::process_uploaded_files( $specific_files, $this->log_file_path );
            ...
            Helpers::set_smartocs_import_data_transient( $this->get_current_importer_data() );
	...
	/**
      ...
      public function import_demo_data_ajax_callback() {
            ...
            if ( ! $use_existing_importer_data ) {
                  ...
                  $this->selected_index = empty( $_POST['selected'] ) ? 0 : absint( wp_unslash( $_POST['selected'] ) );

                  /**
                   * 1). Prepare import files.
                   * Manually uploaded import files or predefined import files via filter: smartocs/import_files
                   */
                  if ( ! empty( $_FILES ) && is_array( $_FILES ) ) { // Using manual file uploads?
                        ...
                        // Set the name of the import files, because we used the uploaded files.
                        $this->import_files[ $this->selected_index ]['import_file_name'] = esc_html__( 'Manually uploaded files', 'smart-one-click-setup' );
                  } elseif ( ! empty( $this->import_files[ $this->selected_index ] ) ) { // Use predefined import files from wp filter: smartocs/import_files.
            ...
            if ( ! $this->before_import_executed ) {
                  ...
                  Helpers::do_action( 'smartocs/before_content_import_execution', $this->selected_import_files, $this->import_files, $this->selected_index );
            }
            ...
            Helpers::do_action( 'smartocs/after_content_import_execution', $this->selected_import_files, $this->import_files, $this->selected_index );

            // Save the import data as a transient.
            ...
            if ( false !== Helpers::has_action( 'smartocs/after_all_import_execution' ) ) {
	...
	public function import_customizer_data_ajax_callback() {
		...
		if ( $this->use_existing_importer_data() ) {
			 ...
			 */
			Helpers::do_action( 'smartocs/customizer_import_execution', $this->selected_import_files );
		}
	...
	public function after_all_import_data_ajax_callback() {
		...
		if ( $this->use_existing_importer_data() ) {
			 ...
			 */
			Helpers::do_action( 'smartocs/after_all_import_execution', $this->selected_import_files, $this->import_files, $this->selected_index );
		}
	...
	/**
	 * Clean up extracted files and directory after import completes.
	 */
	private function cleanup_extracted_files() {
		if ( empty( $this->extract_directory ) || ! file_exists( $this->extract_directory ) ) {
			return;
		}

		// Delete all files in the extract directory.
		$files = glob( trailingslashit( $this->extract_directory ) . '*' );
		if ( is_array( $files ) ) {
		...
		// Delete the extract directory itself using WordPress filesystem API.
		$this->remove_directory( $this->extract_directory );

		// Clear the extract directory path.
		$this->extract_directory = '';
	}
	...
	private function delete_directory( $dir ) {
		...
		// Remove the directory using WordPress filesystem API.
		$this->remove_directory( $dir );
	}

	/**
	 * Remove a directory using WordPress filesystem API.
	 *
	...
	private function remove_directory( $dir ) {
		...
		// Check if WordPress filesystem API is available.
		...
		// Get the upload directory for context.
		$upload_dir = wp_upload_dir();
		$context = ! empty( $upload_dir['basedir'] ) ? dirname( $upload_dir['basedir'] ) : dirname( $dir );

		// Initialize filesystem. Use false to attempt direct filesystem access first.
		// For wp-content/uploads, this typically works without credentials.
		if ( false === WP_Filesystem( false, $context ) ) {
			// If direct access failed, try with empty credentials.
			WP_Filesystem( array(), $context );
		}

		// Use WP_Filesystem to remove directory if available.
		if ( ! empty( $wp_filesystem ) && method_exists( $wp_filesystem, 'rmdir' ) ) {
			// Check if directory exists via filesystem.
			if ( $wp_filesystem->exists( $dir ) ) {
				// Remove directory recursively (second parameter true = recursive).
	...
	private function final_response() {
		// Clean up extracted files after import completes.
		...
		$response['message'] = '<img class="smartocs-imported-content-imported smartocs-imported-content-imported--success" src="' . esc_url( SMARTOCS_URL . 'assets/images/success.svg' ) . '" alt="' . esc_attr__( 'Successful Import', 'smart-one-click-setup' ) . '">';
		...
		if ( ! empty( $this->log_file_path ) ) {
			...
			if ( $has_errors ) {
				...
				$response['subtitle'] .= '<p>' . sprintf(
					...
					wp_kses( $smartocs_log_link, array(
						'a' => [
							'href'   => [],
							'target' => [],
						],
	...
	private function use_existing_importer_data() {
		if ( $data = get_transient( 'smartocs_importer_data' ) ) {
			...
			$this->extract_directory       = empty( $data['extract_directory'] ) ? '' : $data['extract_directory'];
			$this->import_execution_flags  = empty( $data['import_execution_flags'] ) ? array(
				'widgets'          => false,
				'elementor'        => false,
				'redux'            => false,
				'wpforms'          => false,
				'plugin_settings' => false,
			) : $data['import_execution_flags'];
	...
	public function get_current_importer_data() {
		return array(
			...
			'extract_directory'       => $this->extract_directory,
			'import_execution_flags'  => $this->import_execution_flags,
		);
	}


	/**
	 * Check if a specific import type has already been executed.
	 *
	 * @param string $import_type The import type to check (widgets, elementor, redux, wpforms, plugin_settings).
	 * @return bool True if already executed, false otherwise.
	 */
	public function is_import_executed( $import_type ) {
		return ! empty( $this->import_execution_flags[ $import_type ] );
	}

	/**
	 * Mark a specific import type as executed.
	 *
	 * @param string $import_type The import type to mark as executed (widgets, elementor, redux, wpforms, plugin_settings).
	 */
	public function mark_import_executed( $import_type ) {
		if ( isset( $this->import_execution_flags[ $import_type ] ) ) {
			$this->import_execution_flags[ $import_type ] = true;
		}
	}
	...
	public function append_to_frontend_error_messages( $text ) {
		...
		foreach ( $lines as $line ) {
			// Trim whitespace for better duplicate detection
			$line = trim( $line );

			if ( ! empty( $line ) ) {
				// Normalize the line for comparison (remove HTML tags, extra spaces)
				$normalized_line = trim( wp_strip_all_tags( $line ) );

				// Check if a similar line already exists (normalized comparison)
				$is_duplicate = false;
				foreach ( $this->frontend_error_messages as $existing_line ) {
					$normalized_existing = trim( wp_strip_all_tags( $existing_line ) );
					if ( $normalized_line === $normalized_existing && ! empty( $normalized_line ) ) {
						$is_duplicate = true;
						break;
					}
				}
	...
	public function setup_plugin_with_filter_data() {
		...
		$this->import_files = Helpers::validate_import_file_info( Helpers::apply_filters( 'smartocs/import_files', array() ) );

		/**
		 * Register all default actions (before content import, widget, customizer import and other actions)
		 * to the 'before_content_import_execution' and the 'smartocs/after_content_import_execution' action hook.
		 */
		$import_actions = ImportActions::get_instance();
		$import_actions->register_hooks();

		// Importer options array.
		$importer_options = Helpers::apply_filters( 'smartocs/importer_options', array(
			'fetch_attachments' => true,
		) );

		// Logger options for the logger used in the importer.
		$logger_options = Helpers::apply_filters( 'smartocs/logger_options', array(
	 ...
	 * if the smartocs/demo_api_base_url filter is set.
	 *
	...
	public function merge_helper_imports( $predefined_imports ) {
		...
		$api_base_url = Helpers::apply_filters( 'smartocs/demo_api_base_url', '' );
		if ( ! empty( $api_base_url ) ) {
			$api_result = ImportHelper::fetch_from_api();
			// Log errors for debugging if WP_DEBUG is enabled.
			if ( is_wp_error( $api_result ) ) {
				Helpers::debug_log( 'API Error: ' . $api_result->get_error_message() );
			}
	...
	/**
	 * Output the begining of the container div for all notices, but only on SMARTOCS pages.
	 */
	public function start_notice_output_capturing() {
		$screen = get_current_screen();

		// Check for main plugin page or export page.
		if ( false === strpos( $screen->base, $this->plugin_page_setup['menu_slug'] ) && false === strpos( $screen->base, 'smartocs-smart-export' ) ) {
			return;
		}

		echo '<div class="smartocs-notices-wrapper js-smartocs-notice-wrapper">';
	}

	/**
	 * Output the ending of the container div for all notices, but only on SMARTOCS pages.
	 */
	public function finish_notice_output_capturing() {
		...
		echo '</div><!-- /.smartocs-notices-wrapper -->';
	}
	...
	/**
	 * Redirect from the old default SMARTOCS settings page URL to the new one.
	 */
	public function redirect_from_old_default_admin_page() {
		global $pagenow;

		// Verify user capability before processing redirect.
		if ( ! current_user_can( 'import' ) ) {
			return;
		}

		// Verify nonce before accessing GET parameters.
		$nonce_action = 'redirect_old_page_' . get_current_user_id();
		...
		// Verify GET parameter and redirect if it's an outdated page slug.
		if ( $pagenow === 'themes.php' && isset( $_GET['page'] ) ) {
			$page = sanitize_text_field( wp_unslash( $_GET['page'] ) );
			// Only the current slug from plugin_page_setup is supported going forward.
		}
	}
	...
	public function save_wp_navigation_import_mapping( $post_id, $original_id, $postdata, $data ) {
		...
		if ( $postdata['post_type'] !== 'wp_navigation' ) {
			...
			if ( strpos( $postdata['post_content'], '<!-- wp:navigation' ) !== false ) {
				...
				$smartocs_post_nav_block = get_transient( 'smartocs_import_posts_with_nav_block' );

				if ( empty( $smartocs_post_nav_block ) ) {
					$smartocs_post_nav_block = [];
				}

				$smartocs_post_nav_block[] = $post_id;

				set_transient( 'smartocs_import_posts_with_nav_block', $smartocs_post_nav_block, HOUR_IN_SECONDS );
			}
		} else {
			 ...
			 * in transient.
			 */
			$smartocs_menu_mapping = get_transient( 'smartocs_import_menu_mapping' );

			if ( empty( $smartocs_menu_mapping ) ) {
				$smartocs_menu_mapping = [];
			}
			...
			set_transient( 'smartocs_import_menu_mapping', $smartocs_menu_mapping, HOUR_IN_SECONDS );
		}
	}
	...
	public function fix_imported_wp_navigation() {
		...
		$nav_import_mapping = get_transient( 'smartocs_import_menu_mapping' );

		// Get the post IDs that needs to be updated.
		$posts_nav_block = get_transient( 'smartocs_import_posts_with_nav_block' );
	...
	public function export_data_ajax_callback() {
		...
		$has_content = isset( $_POST['export_content'] ) && ! empty( $_POST['export_content'] );
		$has_widgets = isset( $_POST['export_widgets'] ) && ! empty( $_POST['export_widgets'] );
		$has_customizer = isset( $_POST['export_customizer'] ) && ! empty( $_POST['export_customizer'] );
		$has_elementor = isset( $_POST['export_elementor'] ) && ! empty( $_POST['export_elementor'] );
		$has_plugins = isset( $_POST['export_plugins'] ) && ! empty( $_POST['export_plugins'] );
		...
		// Read and sanitize value from POST; nonce is verified at the top of this method.
		// For JSON input, we use filter_input() with FILTER_SANITIZE_FULL_SPECIAL_CHARS to sanitize input
		// (as required - must use a sanitizing filter, not FILTER_UNSAFE_RAW or FILTER_DEFAULT).
		// Note: FILTER_SANITIZE_FULL_SPECIAL_CHARS HTML-encodes special characters, which would break JSON.
		// However, we then use sanitize_textarea_field() which properly sanitizes JSON strings without
		// HTML-encoding, preserving JSON structure while ensuring security.
		$raw_custom_plugin_options = filter_input( INPUT_POST, 'custom_plugin_options', FILTER_SANITIZE_FULL_SPECIAL_CHARS );

		if ( ! empty( $raw_custom_plugin_options ) ) {
			// Validate input is a string before processing.
			if ( ! is_string( $raw_custom_plugin_options ) ) {
				wp_send_json_error( esc_html__( 'Invalid custom plugin options format.', 'smart-one-click-setup' ) );
			}
			// Apply WordPress sanitization function which properly sanitizes JSON strings.
			// sanitize_textarea_field() sanitizes input while preserving JSON structure (doesn't HTML-encode).
			// This provides defense-in-depth: filter_input() sanitizes with FILTER_SANITIZE_FULL_SPECIAL_CHARS,
			// then WordPress function sanitizes again in a way that preserves JSON structure.
			$custom_options_json = sanitize_textarea_field( wp_unslash( $raw_custom_plugin_options ) );
			// Validate that it's a non-empty string.
			if ( empty( $custom_options_json ) ) {
				wp_send_json_error( esc_html__( 'Invalid custom plugin options format.', 'smart-one-click-setup' ) );
			}
			// Decode JSON and validate it's an array.
			// Security: Input is sanitized above, and decoded data is immediately sanitized again below.
			// This ensures json_decode() is never relied upon for sanitization and any potentially
			// unsafe data is cleaned before use.
			$custom_options_decoded = json_decode( $custom_options_json, true );
			// Validate JSON decode was successful.
			if ( json_last_error() !== JSON_ERROR_NONE || ! is_array( $custom_options_decoded ) ) {
				wp_send_json_error( esc_html__( 'Invalid JSON format in custom plugin options.', 'smart-one-click-setup' ) );
			}
			// Sanitize decoded data recursively so that json_decode() output is never used unsanitized.
			// String values are sanitized with sanitize_text_field, while non-strings are left intact
			// and further validated/sanitized in the processing loop below.
			$custom_options_decoded = map_deep(
				$custom_options_decoded,
					...
					if ( is_array( $options_data ) && isset( $options_data['options'] ) ) {
						...
						if ( $is_object_format && is_array( $options ) ) {
							...
							foreach ( $options as $option_name => $option_value ) {
								...
								// Sanitize option values based on their type.
								if ( is_string( $option_value ) ) {
									$sanitized_options[ $option_name ] = sanitize_text_field( $option_value );
								} elseif ( is_numeric( $option_value ) ) {
									$sanitized_options[ $option_name ] = $option_value;
								} elseif ( is_bool( $option_value ) ) {
									$sanitized_options[ $option_name ] = (bool) $option_value;
								} elseif ( is_array( $option_value ) ) {
									// Recursively sanitize array values using WordPress helper.
									$sanitized_options[ $option_name ] = map_deep( $option_value, 'sanitize_text_field' );
								} else {
									// Skip invalid types.
									continue;
								}
		...
		$export_options = Helpers::apply_filters( 'smartocs/export_options', $export_options );

		try {
			$exporter = new \SMARTOCS\Exporter( $export_options );
			...
			wp_send_json_error( sprintf( esc_html__( 'Export failed: %s', 'smart-one-click-setup' ), esc_html( $e->getMessage() ) ) );
		} catch ( \Error $e ) {
			/* translators: %s: The error message. */
			wp_send_json_error( sprintf( esc_html__( 'Export failed: %s', 'smart-one-click-setup' ), esc_html( $e->getMessage() ) ) );
		}
	...
	public function import_zip_file_ajax_callback() {
		...
		// Extract only the specific key we need, not the entire $_FILES array.
		$files = array();
		if ( isset( $_FILES['zip_file'] ) && is_array( $_FILES['zip_file'] ) ) {
			$files['zip_file'] = array(
				...
				'type'     => isset( $_FILES['zip_file']['type'] ) ? sanitize_mime_type( wp_unslash( $_FILES['zip_file']['type'] ) ) : '',
				'tmp_name' => isset( $_FILES['zip_file']['tmp_name'] ) ? sanitize_text_field( wp_unslash( $_FILES['zip_file']['tmp_name'] ) ) : '',
				'error'    => isset( $_FILES['zip_file']['error'] ) ? absint( wp_unslash( $_FILES['zip_file']['error'] ) ) : 0,
				'size'     => isset( $_FILES['zip_file']['size'] ) ? absint( wp_unslash( $_FILES['zip_file']['size'] ) ) : 0,
			);
		}
		$validated_file = $this->get_validated_file_upload( 'zip_file', $files );
		if ( false === $validated_file ) {
			wp_send_json_error( esc_html__( 'No ZIP file provided or invalid file upload structure.', 'smart-one-click-setup' ) );
		}

		// Validate file type before processing.
		if ( ! empty( $validated_file['file'] ) ) {
			$file_ext = strtolower( pathinfo( $validated_file['file'], PATHINFO_EXTENSION ) );
			if ( 'zip' !== $file_ext ) {
				// Cleanup uploaded file if invalid type
				if ( file_exists( $validated_file['file'] ) ) {
					wp_delete_file( $validated_file['file'] );
				}
				wp_send_json_error( esc_html__( 'Invalid file type. Only ZIP files are allowed.', 'smart-one-click-setup' ) );
			}
			
			// Additional mime type check
			$file_type = isset( $validated_file['type'] ) ? $validated_file['type'] : '';
			$allowed_mimes = array( 'application/zip', 'application/x-zip-compressed', 'multipart/x-zip', 'application/x-compressed' );
		...
		$zip_file = $validated_file['file'];

		// Extract ZIP file.
		$extracted_files = $this->extract_zip_file( $zip_file );
		if ( is_wp_error( $extracted_files ) ) {
			wp_send_json_error( esc_html( $extracted_files->get_error_message() ) );
		}

		// Execute before import action hook (not user-provided code for security).
		// Developers can hook into this via add_action( 'smartocs/before_content_import', ... ).
		Helpers::do_action( 'smartocs/before_content_import', $extracted_files );
		...
		// Execute after import action hook (not user-provided code for security).
		// Developers can hook into this via add_action( 'smartocs/after_import', ... ).
		Helpers::do_action( 'smartocs/after_import', $extracted_files );
	...
	public function import_predefined_zip_ajax_callback() {
		...
		$import_index = isset( $_POST['import_index'] ) ? absint( wp_unslash( $_POST['import_index'] ) ) : -1;
		...
		elseif ( ! empty( $import_config['zip_url'] ) ) {
			...
			if ( is_wp_error( $zip_file_path ) ) {
				/* translators: %s: The error message. */
				wp_send_json_error( sprintf( esc_html__( 'Failed to download ZIP file: %s', 'smart-one-click-setup' ), esc_html( $zip_file_path->get_error_message() ) ) );
			}
		}
		else {
		...
		if ( ! empty( $import_config['after_import'] ) && is_callable( $import_config['after_import'] ) ) {
			...
			add_action( 'smartocs/after_import', function( $selected_import_files, $import_files, $selected_index ) use ( $import_config, $current_index ) {
	...
	private function extract_zip_file( $zip_file_path ) {
		...
		$upload_dir = wp_upload_dir();
		$extract_dir = trailingslashit( $upload_dir['basedir'] ) . 'smartocs-extracted/' . basename( $zip_file_path, '.zip' ) . '/';

		// Store extract directory for cleanup later.
		$this->extract_directory = $extract_dir;
	...
	$buttons_html = '';
		foreach ( $buttons as $button ) {
			...
			if ( strpos( $class, 'smartocs-success-button' ) === false ) {
				$class .= ' smartocs-success-button';
			}

			// Build additional attributes array for safe output.
			$additional_attrs_array = array();
			if ( ! empty( $button['attributes'] ) && is_array( $button['attributes'] ) ) {
				foreach ( $button['attributes'] as $attr_name => $attr_value ) {
					...
					if ( ! empty( $attr_name ) ) {
						$additional_attrs_array[ $attr_name ] = $attr_value;
					}
				}
			}
			...
			$button_label = apply_filters( 'smartocs/import_successful_button_label', $button['label'], $button );
			 ...
			 */
			// phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.DynamicHooknameFound
			$button_href = apply_filters( 'smartocs/import_successful_button_href', $button['href'], $button );

			// Build the complete anchor tag with properly escaped attributes.
		$button_html = '<a href="' . esc_url( $button_href ) . '" class="' . esc_attr( $class ) . '" target="' . esc_attr( $target ) . '"';
			// Output additional attributes with proper escaping.
			if ( ! empty( $additional_attrs_array ) ) {
				foreach ( $additional_attrs_array as $attr_name => $attr_value ) {
				$button_html .= ' ' . esc_attr( $attr_name ) . '="' . esc_attr( $attr_value ) . '"';
				}
			}
		$button_html .= '>' . esc_html( $button_label ) . '</a>';
	...
	/**
       * Safely extract and validate a file from a files array and process it securely.
       * * NOTE: This method now uses wp_handle_upload() to comply with security standards.
       *
       * @param string $file_key The key in the files array.
       * @param array  $files    The files array to extract the file from (e.g. a local copy of $_FILES).
       * @return array|false Sanitized, validated, and successfully moved file data array, or false if invalid.
       */
	
      	private function get_validated_file_upload( $file_key, $files ) {
            // 1. Structural Checks (Kept simple for validation)
            if ( ! is_string( $file_key ) || empty( $file_key ) || ! is_array( $files ) || empty( $files ) ) {
                  return false;
            }

            // Check if files array has the expected key.
            if ( ! isset( $files[ $file_key ] ) || ! is_array( $files[ $file_key ] ) ) {
                  return false;
            }

            $file_entry = $files[ $file_key ];

            // Check for required keys and successful upload before processing.
            if ( ! isset( $file_entry['tmp_name'] ) || $file_entry['error'] !== UPLOAD_ERR_OK ) {
                  return false;
            }

            // 2. Prepare File Data for wp_handle_upload()
            // We use the existing, partially validated structure:
            $file_data = array(
                  'name'     => sanitize_file_name( $file_entry['name'] ), // Sanitize file name
                  'type'     => sanitize_mime_type( $file_entry['type'] ), // Sanitize MIME type
                  'tmp_name' => $file_entry['tmp_name'], 
                  'error'    => UPLOAD_ERR_OK, // Assume 0 as validated above
                  'size'     => absint( $file_entry['size'] ?? 0 ),
            );
            
            // Note: download_url and wp_handle_upload should be available when WordPress is loaded.
            // If they're not, it's likely a WordPress installation issue.

            // 4. Secure Processing with wp_handle_upload()
            // This function handles the is_uploaded_file() check, validation, and safe file move.
            $upload_overrides = array( 
                  'test_form' => false, // Crucial for AJAX/non-standard forms
                  'action'    => 'smartocs_manual_import', // Unique action for filtering
                  // NOTE: You may need to specify 'mimes' here if the file types are restricted beyond the default XML/JSON.
            );

            $uploaded_file = wp_handle_upload( $file_data, $upload_overrides );

            // 5. Check for Errors from wp_handle_upload()
            if ( isset( $uploaded_file['error'] ) ) {
                  // The upload or validation failed (e.g., file type rejected by WordPress core).
                  return false;
            }

            // Return the sanitized, validated, and safely moved file data array.
            return $uploaded_file;
      }


	/**
	 * Validate and sanitize file upload array.
	 *
	 * @param array $file_array The file upload array to validate and sanitize.
	 * @return array|false Validated and sanitized file array, or false if invalid.
	 */
	private function validate_and_sanitize_file_array( $file_array ) {
