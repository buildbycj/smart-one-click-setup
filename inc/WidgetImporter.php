...
class WidgetImporter {
	...
	public static function import( $widget_import_file_path ) {
		...
		if ( is_wp_error( $results ) ) {
			...
			Helpers::append_to_file(
				$error_message,
				$log_file_path,
				esc_html__( 'Importing widgets', 'smart-one-click-setup' )
			);
		}
		else {
			ob_start();
	...
	private static function import_data( $data ) {
		...
		// Hook before import.
		Helpers::do_action( 'socs/widget_importer_before_widgets_import' );
		$data = Helpers::apply_filters( 'socs/before_widgets_import_data', $data );
		...
		foreach ( $data as $sidebar_id => $widgets ) {
			...
			foreach ( $widgets as $widget_instance_id => $widget ) {
				...
				$widget = Helpers::apply_filters( 'socs/widget_settings', $widget ); // Object.
				...
				$widget = Helpers::apply_filters( 'socs/widget_settings_array', $widget );
				...
				if ( ! $fail ) {
					...
					Helpers::do_action( 'socs/widget_importer_after_single_widget_import', $after_widget_import );
		...
		// Hook after import.
		Helpers::do_action( 'socs/widget_importer_after_widgets_import' );

		// Return results.
		return Helpers::apply_filters( 'socs/widget_import_results', $results );
	}
	...
	private static function available_widgets() {
		...
		return Helpers::apply_filters( 'socs/available_widgets', $available_widgets );
	}
	...
	private static function format_results_for_log( $results ) {
		if ( empty( $results ) ) {
			esc_html_e( 'No results for widget import!', 'smart-one-click-setup' );
		}
